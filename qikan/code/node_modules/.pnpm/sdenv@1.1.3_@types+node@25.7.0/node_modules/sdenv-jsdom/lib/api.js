...
class JSDOM {
  constructor(input = "", options = {}) {
    ...
    const renderBodyFlagCode = '<script script-type="sdenv">(window.sdenv ? window.sdenv.memory.setTimeout : setTimeout)(() => {window.document.renderBodyFlag = true}, 0);</script>'
    parseIntoDocument(html.replace('<body>', `<body>${renderBodyFlagCode}`), documentImpl);
